...
/**
 * Asynchronously closes the document currently opened in PTPDFViewCtrl.
 * This method "invalidates" the `PTPDFViewCtrl`, and it can no longer be
 * used once this method is called. (A new `PTPDFViewCtrl` must be used.)
 * This method is primarily intended for use with `OpenURLAsyc:`, when calling
 * `CloseDoc` can under certain workflows block the main thread for a second or two.
 *
 * @note CloseDocAsync acquires a write lock on the document, so the document
 * must be unlocked when this method is called or an exception will be
 * thrown.
 */
-(void)CloseDocAsync;
 ...
 */
FOUNDATION_EXPORT const NSNotificationName PTPDFViewCtrlPageDidChangeNotification;
 ...
 * is empty.
 */
FOUNDATION_EXPORT const NSNotificationName PTPDFViewCtrlDidCloseDocNotification;


/**
 * Posted when the PTPDFViewCtrl has streaming event notification. These are generated when downloading
 * a document via `openUrlAync:`, and when opening a non-PDF document with `openUniversalDocumentWithConversion`.
 *
 * The notification object is the pdfViewCtrl that posted the notification. The `userInfo` dictionary
 * will contain PTPDFViewCtrlStreamingEventTypeUserInfoKey and PTPDFViewCtrlPageNumberUserInfoKey keys.
 */
FOUNDATION_EXPORT const NSNotificationName PTPDFViewCtrlStreamingEventNotification;
 ...
 */
FOUNDATION_EXPORT NSString * const PTPDFViewCtrlPreviousPageNumberUserInfoKey;

/**
 * The key for a `NSNumber` object containing an `int` page number.
 */
FOUNDATION_EXPORT NSString * const PTPDFViewCtrlCurrentPageNumberUserInfoKey;

/**
 * The key for a `NSNumber` object containing an `int` page number.
 */
FOUNDATION_EXPORT NSString * const PTPDFViewCtrlPageNumberUserInfoKey;

/**
 * The key for a `NSNumber` object containing a PTDownloadedType.
 */
FOUNDATION_EXPORT NSString * const PTPDFViewCtrlStreamingEventTypeUserInfoKey;
