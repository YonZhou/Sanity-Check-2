...
typedef enum PTCloudErrorCode {  PTSTATUS_ERR,
  PTSTATUS_OK,
  PTSTATUS_NETWORK_ERR,
  PTSTATUS_BAD_CREDENTIALS,
  PTSTATUS_SERVICE_DOWN,
  PTSTATUS_INVALID_OPERATION,
  PTSTATUS_NUM}PTCloudErrorCode;
...
@interface PTVectorString : NSObject
{
	void *swigCPtr;
	BOOL swigCMemOwn;
}
...
+ (void)ToTiff: (NSString*)in_filename out_path:  (NSString*)out_path;
...
- (void)setSwigCMemOwn: (BOOL) own;
- (void)dealloc;
- (void)WriterBeginWithPage: (PTPage*)page placement:  (PTWriteMode)placement page_coord_sys:  (BOOL)page_coord_sys compress:  (BOOL)compress;
- (void)Begin: (PTPage*)page placement:  (PTWriteMode)placement page_coord_sys:  (BOOL)page_coord_sys;
- (void)WriterBeginWithSDFDoc: (PTSDFDoc*)doc compress:  (BOOL)compress;
- (void)WriterBeginWithSDFObj: (PTObj*)stream_obj_to_update compress:  (BOOL)compress;
- (PTObj*)End;
...
+ (double)GetVersion;
+ (PTCloudErrorCode)ConnectToCloud: (NSString *)username password:  (NSString *)password demo_mode:  (BOOL)demo_mode;
